...
def ws_message(message):
	...
	#Game.objects.raw('DELETE * FROM game_game_players');
	#Game.objects.raw('DELETE * FROM game_gameplayer');
	...
	if "action" not in obj:
		...
		if obj['action'] == 'join':
			...
			obj['gameIsFull'] = addPlayer(game,obj['username'])
			Group("player").send({'text': json.dumps(obj)}) # Va nous servir à update l'UI

		elif obj['action'] == 'draw':
			draw(game,obj['username'],obj['number'])
			Group("player").send({'text': json.dumps(obj)}) # Va nous servir à update l'UI

		elif obj['action'] == 'put':
			put(game,obj['username'],obj['card'])
			Group("player").send({'text': json.dumps(obj)}) # Va nous servir à update l'UI
		elif obj['action'] == 'attack':
			attack(game,obj['username'],obj['attackingCard'],obj['target'])
			Group("player").send({'text': json.dumps(obj)}) # Va nous servir à update l'UI

		elif obj['action'] == 'disconnect':
			removePlayer(game,obj['username'])
